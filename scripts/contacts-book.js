...
export async function renderContacts(account, fPrompt = false) {
    ...
    if (!fPrompt) {
        ...
        strHTML += `
            <div class="shadowInnerCard" style="font-family: 'Montserrat'; text-align: start; margin: 0px 15px; border-radius: 10px; border: 1px solid #11777e; background-color: #183c3d; padding: 13px 13px; margin-bottom: 28px;">
                <label style="color:#9cc6c6; font-size: 15px; font-weight: 500; margin-bottom: 17px;">Add new contact</label>
        ...
        }" style="font-family: 'Montserrat'; text-align: start; margin: 0px 15px; border-radius: 10px; border: 1px solid #11777e; background-color: #183c3d; padding: 13px 9px;">
        ...
        for (const cContact of account.contacts || []) {
            const strPubkey = isXPub(cContact.pubkey)
                ...
                : cContact.pubkey;
            strHTML += `
            <div class="d-flex px-3 py-3 contactItem" style="padding-left: 10px !important; border-bottom: 1px solid #15847c;">
                <div>
                    <img onclick="MPW.guiAddContactImage('${i}')" class="ptr" style="margin-right:20px; width: 50px; height: 50px; border-radius: 100%; background-color: white; border: 2px solid #adfff0;" ${
                cContact.icon ? 'src="' + cContact.icon + '"' : ''
            }>
                </div>
                <div style="width: 100%; line-height: 15px; margin-top: -6px;">
                    <span id="contactsName${i}" onclick="MPW.guiEditContactNamePrompt('${i}')" style="word-wrap: anywhere; cursor:pointer; color: #adfff0; font-weight: 500; margin-top: 8px; display: block;">${sanitizeHTML(
                cContact.label
            )}</span>
                    <span id="contactsAddress${i}" style="word-wrap: anywhere; font-size: 13px; position: relative; top: 10px;">
                        <code style="background-color: #122d2b; color: #659e99; padding: 7px 10px; font-size: 13px; border-radius: 9px;">
        ...
        for (const cContact of account.contacts || []) {
            const strPubkey = isXPub(cContact.pubkey)
                ...
                : cContact.pubkey;
            strHTML += `
            <div class="d-flex px-3 py-3 contactItem ptr" id="contactsSelector${i}" style="border-bottom: 1px solid #15847c;">
                <div id="contactsAvatarContainer${i}">
                    <img id="contactsAvatar${i}" class="ptr" style="margin-right:20px; width: 50px; height: 50px; border-radius: 100%; background-color: white; border: 2px solid #adfff3;" ${
                cContact.icon ? 'src="' + cContact.icon + '"' : ''
            }>
                </div>
                <div id="contactsNameContainer${i}" style="width: 100%; line-height: 15px; margin-top: -6px;">
                    <span id="contactsName${i}" style="word-wrap: anywhere; color: #adfff3; font-weight: 600; margin-top: 8px; display: block;">${sanitizeHTML(
                sanitizeHTML(cContact.label)
            )}</span>
                    <span style="word-wrap: anywhere; font-size: 13px; position: relative; top: 10px;">
                        <code id="contactsAddress${i}" style="background-color: #122d2b; color: #659e99; padding: 7px 10px; font-size: 13px; border-radius: 9px;">
...
async function renderContactModal() {
    ...
    if (cAccount?.name) {
        ...
        if (await hasEncryptedWallet()) {
            doms.domModalQR.innerHTML = `
                <b style="margin-bottom: 9px; display: block; font-size: 16px; color:#9cc6c6; font-weight: 500;">${translation.setupYourContact}</b>
...
function renderAddress(strAddress) {
    ...
    try {
        createQR('zenzo:' + strAddress, doms.domModalQR);
        domQR.firstChild.style.width = '100%';
        domQR.firstChild.style.height = 'auto';
        domQR.firstChild.classList.add('no-antialias');
    } catch (e) {
