 ...
 * @property {string} name - ZENZO address string
 ...
 * @property {number} decimals - Decimal places in the amounts (ZENZO has 8 decimals)
...
export class RPCNodeNetwork extends Network {
     ...
     * @param {String} options.address - Base58 encoded ZENZO address
...
export class ExplorerNetwork extends Network {
     ...
     * @param {string} addr - a ZENZO address or xpub
    ...
    async #getPage(nStartHeight, addr, n, pageSize) {
        if (!(isXPub(addr) || isStandardAddress(addr))) {
            throw new Error('must provide either a ZENZO address or a xpub');
        }
