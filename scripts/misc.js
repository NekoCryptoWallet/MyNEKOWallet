 ...
 * @return {boolean} If a straddress is a valid ZENZO address,
...
export function parseBIP21Request(strReq) {
    // Format should match: zenzo:addr[?amount=x&label=x]
    if (!strReq.includes(BIP21_PREFIX + ':')) return false;

    const [addressPart, optionsPart] = strReq.includes('?')
        ? strReq.split('?')
        : [strReq, false];
    const strAddress = addressPart.substring(BIP21_PREFIX.length + 1); // remove 'zenzo:' prefix
    let cOptions = {};
