 ...
 */
export const ORACLE_BASE = 'https://api.nekosunevr.co.uk/v5/cryptoapi/nekogeko';
...
export class Oracle {
    ...
    getCachedPrice(strCurrency) {
        return this.mapCurrencies.get(strCurrency)?.current_price || 0;
    }
    ...
    async getPrice(strCurrency) {
        try {
            const cReq = await fetch(`${ORACLE_BASE}/prices/${strCurrency}?id=zenzo`);
            ...
            return cCurrency.current_price;
        } catch (e) {
            debugWarn(
                DebugTopics.NET,
    ...
    async getCurrencies() {
        try {
            const cReq = await fetch(`${ORACLE_BASE}/currencies?id=zenzo`);
